...
void PrintGlobals(void (*func)(symtabnode *t))
{
  int i;
  for (i = 0; i < HASHTBLSZ; i++)
  {
...
int GetTotalLocalBytes()
{
  ...
  for (i = 0; i < HASHTBLSZ; i++)
  {
    ...
    {
      if (stptr != NULL && !stptr->formal)
      {
        if (stptr->type == t_Char)
          bytes += 1;
        else if (stptr->type == t_Int)
        {
          while (bytes % 4 != 0)
            bytes++;
          bytes += 4;
 ...
 * GetTotalLocalBytes()
 */
int GetFrameOffset(symtabnode *t, symtabnode *function)
{
  ...
  if (t->formal)
  {
    for (stptr = function->formals; stptr != NULL; stptr = stptr->next)
    {
      formal_offset += 4;
      if (strcmp(t->name, stptr->name) == 0)
        return formal_offset;
    }
  }
...
symtabnode *SymTabInsert(char *str, int sc)
{
  ...
  if (sc == Global)
  {
    sptr->fn_proto_state = 0;
    sptr->global_printed = 0;
  }
