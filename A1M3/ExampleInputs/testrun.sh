...
do
    ...
    [[ $gen_mips = true ]] && ./compile -DDEBUG < "${test_prefix}${i}.c" > "${mips_prefix}${i}.s" 2> "${stderr_prefix}${i}.txt"
    [[ $valgrind = true ]] && valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose --log-file="${valgrind_prefix}${i}.txt" ./compile -DDEBUG < "${test_prefix}${i}.c" > "${mips_prefix}${i}.s" 2> "${stderr_prefix}${i}.txt"
    [[ $run_spim = true ]] && /home/cs553/spring21/bin/spim -keepstats -file "${mips_prefix}${i}.s" > "${spim_prefix}${i}.txt"
